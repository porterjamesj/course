...
instance Functor ListZipper where
  g <$>(ListZipper r f l) = ListZipper (g <$> r) (g f) (g <$> l)
...
toList ::
  ListZipper a
  -> List a
toList (ListZipper l f r) = (reverse l) ++ (f:.Nil) ++ r
...
findLeft ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findLeft p (ListZipper l f r) = case (break p l) of
  (_, Nil) -> IsNotZ
  (before, (found:.after)) -> IsZ $ ListZipper after found ((reverse before) ++ (f:.Nil) ++ r)
...
findRight ::
  ...
  -> ListZipper a
  -> MaybeListZipper a
findRight p (ListZipper l f r) = case (break p r) of
  (_, Nil) -> IsNotZ
  (before, (found:.after)) -> IsZ $ ListZipper ((reverse before) ++ (f:.Nil) ++ l) found after

-- This is very dissapointing. What would it take to be able to
-- abstract over right vs left?
...
moveLeftLoop (ListZipper Nil f r) = ListZipper (rest ++ (f:.Nil)) head Nil
  where (head:.rest) = reverse r
moveLeftLoop (ListZipper (head:.rest) f r) = ListZipper rest head (f:.r)
...
moveRightLoop (ListZipper l f Nil) = ListZipper Nil head (rest ++ (f:.Nil))
  where (head:.rest) = reverse l
moveRightLoop (ListZipper l f (head:.rest)) = ListZipper (f:.l) head rest
...
swapLeft (ListZipper (head:.rest) f r) = IsZ $
  ListZipper (f:.rest) head r
...
swapRight (ListZipper l f (head:.rest)) = IsZ $
  ListZipper l head (f:.rest)
...
moveLeftN n lz
  | n == 0 = IsZ $ lz
  | n < 0 = moveRightN (-n) lz
  | otherwise = moveLeftN (n-1) -<< (moveLeft lz)
...
moveRightN n lz
  | n == 0 = IsZ $ lz
  | n < 0 = moveLeftN (-n) lz
  | otherwise = moveRightN (n-1) -<< (moveRight lz)
...
moveLeftN' n lz@(ListZipper l _ _)
...
moveRightN' ::
  ...
  -> ListZipper a
  -> Either Int (ListZipper a)
moveRightN' n lz@(ListZipper _ _ r)
...
start (ListZipper l f r) = ListZipper
  Nil head (rest ++ (f:.Nil) ++ r)
  where
    (head:.rest) = reverse l
...
deletePullLeft (ListZipper (head:.rest) _ r) =
  IsZ $ ListZipper rest head r
...
deletePullRight (ListZipper l _ (head:.rest)) =
  IsZ $ ListZipper l head rest
...
insertPushLeft ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushLeft a (ListZipper l f r) = ListZipper (f:.l) a r
...
insertPushRight ::
  ...
  -> ListZipper a
  -> ListZipper a
insertPushRight a (ListZipper l f r) = ListZipper l a (f:.r)
...
-- /Tip:/ Use `zipWith`
  (ListZipper lfs ff rfs) <*> (ListZipper las fa ras) =
    ListZipper (zipWith ($) lfs las) (ff fa) (zipWith ($) rfs ras)
