...
seenT :: (Ord a, Monad f) => a -> StateT (S.Set a) f Bool
seenT a = do
  s <- getT
  putT $ S.insert a s
  pure $ S.member a s
...
distinct' ::
  ...
  List a
  -> List a
distinct' = (flip eval') S.empty . filtering ((not <$>) . seenT)
...
distinctF ::
  ...
  List a
  -> Optional (List a)
distinctF = (flip evalT) S.empty . filtering run
  where run n = StateT $ \s ->
          if n > 100 then Empty else Full (not $ S.member n s, S.insert n s)
...
instance Monad f => Monad (OptionalT f) where
  famb =<< (OptionalT fa) = OptionalT $ do
    a <- fa
    case a of
      Empty -> pure Empty
      Full a' -> runOptionalT $ famb a'
...
instance Applicative (Logger l) where
  pure = Logger Nil
  (Logger l1 fab) <*> (Logger l2 a) = Logger (l1 ++ l2) (fab a)
